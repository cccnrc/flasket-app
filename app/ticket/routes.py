...
def submit():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        send_ticket_email(ticket.author, ticket, object = 'SUBMIT')
...
def ticket_reply(ticket_ID):
    ...
    if form.validate_on_submit():
        ...
        if body:
            ...
            ticket.ticket_followers.append(user)
            send_ticket_email(ticket.author, ticket_reply, object = 'UPDATE')
            for single_user in ticket.ticket_followers:
                send_ticket_email(single_user, ticket_reply, object = 'UPDATE')
...
def close(ticket_ID):
    ...
    send_ticket_email(ticket.author, ticket, object = 'CLOSURE')
    for single_user in ticket.ticket_followers:
        send_ticket_email(single_user, ticket, object = 'CLOSURE')
    db.session.commit()
    return( redirect( url_for('ticket.ticket_reply', ticket_ID = ticket.id )))
...
def reopen(ticket_ID):
    ...
    ticket.closed = False
    ticket.last_modify = datetime.utcnow()
    send_ticket_email(ticket.author, ticket, object = 'REOPEN')
    for single_user in ticket.ticket_followers:
        send_ticket_email(single_user, ticket, object = 'REOPEN')
    db.session.commit()
    return( redirect( url_for('ticket.ticket_reply', ticket_ID = ticket.id )))
